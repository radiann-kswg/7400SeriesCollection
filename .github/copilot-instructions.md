...
2. **一次資料はリポジトリ内 PDF を最優先**

- `datasheets/_download/` 配下に対象メーカーの PDF がある場合、ネット情報より優先します。

3. **PDF が“画像主体”でテキスト抽出できない場合の手順**
...
5. **入手済みパーツ（実型番）を優先し、対応 PDF の有無を確認する**

- 回路例で使用 IC を選ぶ場合は、まず `getstarting/7400_series_ic_collection_acquisition_history.json` の `GottenItemsMN_*` に含まれる実型番を優先します。
- ピン番号・信号名・真理値表など「実配線に直結する記述」は、対応するメーカー/型番/パッケージの一次資料（リポジトリ内 PDF）を確認できた場合のみ記述します。

6. **キリル型番等の互換パーツは、例外としてネット情報を優先できる（ただし共同確認前提）**
