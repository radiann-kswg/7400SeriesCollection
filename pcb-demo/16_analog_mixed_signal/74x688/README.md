...
| -------------------- | ------------------ |
| **汎用型番**         | `74x688`  |
| **機能 (EN)**        | 8-bit magnitude comparator, equality    |
| **機能 (JP)**        | 8ビットマグニチュードコンパレータ（等価判定） |
| **カテゴリ**         | `16_analog_mixed_signal`     |
| **入手済み (CMOS)**  | SN74HC688N     |
| **入手済み (TTL)**   | (なし)      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x688.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x688.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x688.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x688.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/16_analog_mixed_signal/74x688/`](../../../usage/16_analog_mixed_signal/74x688/)
...
- [ ] `usage/16_analog_mixed_signal/74x688/` のファクトチェックが完了している
