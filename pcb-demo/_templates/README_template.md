...
| **カテゴリ**         | `{{CATEGORY}}`     |
| **入手済み (CMOS)**  | {{CMOS_PARTS}}     |
| **入手済み (TTL)**   | {{TTL_PARTS}}      |
| **入手済み (Other)** | {{OTHER_PARTS}}    |

## KiCAD プロジェクト構成

| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `{{PART_NUMBER}}.kicad_pro` | KiCAD プロジェクトファイル                       |
| `{{PART_NUMBER}}.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `{{PART_NUMBER}}.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `{{PART_NUMBER}}.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/{{CATEGORY}}/{{PART_NUMBER}}/`](../../../usage/{{CATEGORY}}/{{PART_NUMBER}}/)
...
- [ ] `usage/{{CATEGORY}}/{{PART_NUMBER}}/` のファクトチェックが完了している
