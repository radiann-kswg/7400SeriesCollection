...
| -------------------- | ------------------ |
| **汎用型番**         | `74x162`  |
| **機能 (EN)**        | Synchronous presettable 4-bit decade counter, synchronous clear    |
| **機能 (JP)**        | 同期式プリセット可能4ビット10進カウンタ（同期クリア） |
| **カテゴリ**         | `14_system_controllers_timing`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | DM74LS162AN      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x162.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x162.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x162.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x162.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/14_system_controllers_timing/74x162/`](../../../usage/14_system_controllers_timing/74x162/)
...
- [ ] `usage/14_system_controllers_timing/74x162/` のファクトチェックが完了している
