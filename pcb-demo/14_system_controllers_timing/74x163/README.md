...
| -------------------- | ------------------ |
| **汎用型番**         | `74x163`  |
| **機能 (EN)**        | 4-bit binary counter, synchronous, with clear and load    |
| **機能 (JP)**        | 4ビットバイナリカウンタ（同期式、クリア・ロード付き） |
| **カテゴリ**         | `14_system_controllers_timing`     |
| **入手済み (CMOS)**  | SN74HC163N     |
| **入手済み (TTL)**   | 74LS163AN, SN74S163N      |
| **入手済み (Other)** | DM74163AN    |

## KiCAD プロジェクト構成

| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x163.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x163.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x163.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x163.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/14_system_controllers_timing/74x163/`](../../../usage/14_system_controllers_timing/74x163/)
...
- [ ] `usage/14_system_controllers_timing/74x163/` のファクトチェックが完了している
