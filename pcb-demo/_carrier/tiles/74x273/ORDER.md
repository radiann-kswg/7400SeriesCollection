# JLCPCB 発注チェックリスト — 74x273

**全項目が `[x]` になるまで発注しない。** これが発注ゲート（[`REQUIREMENTS.md`](../../../REQUIREMENTS.md) D11）。

`ORDER_CHECKLIST.md`（テンプレート）を 74x273 用にコピーしたもの。2026-09-17、D25（入力接点スイッチのみ化）
適用後の再検証・製造データ作成の結果を記録する。**D26により非パネル（1枚=1回路）で発注する**
（JLCPCB PCBAビューアでパネル化Gerberに対しBOM/CPLの自動複製が機能しなかったため）。

- 対象型番: 74x273（DIP-20、VCC=接点20 GND=接点10、入力セル9/出力セル8/DNP0、クロックセル1（D17整形回路、LED表示は対象外でD17のまま維持））
...
- [x] 対象型番について VCC / GND のピン位置を一次資料で確認した（`_carrier/pinspec/74x273.json`。D15以前に確認済み、本セッションでは未変更）
- [x] 対象型番についてピン数を確認した（`DIP-20`。`load_part()` が生成時に検証）
- [x] `_carrier/pinspec/74x273.json` があり、各ピンの方向を一次資料で確認した
- [x] 電源接点に I/O セルが載っていない（`load_part()` が VCC/GND の一意性を生成時に検証）
- [x] 未使用接点の I/O セルが DNP 指定されている（DNP0個。本セッションBOM/DNP突合で確認）
...
- [x] `Rd` が 220Ω（D16。470Ω 以下への変更なし。`74x273_bom.csv` で確認）
...
- [x] ERC エラー 0（本セッション、74x273 タイル再生成後に再実行）
...
- [x] 全部品に LCSC 部品番号（`Cxxxxx`）が付いている（BOM 記載 10 種すべて。手はんだの `U1`・`J1` は LCSC番号なしでBOM/CPLから除外——設計上の意図どおり）
...
- [x] 抵抗値が 220Ω（Rd）/ 4.7kΩ（Rled）の 2 種類に収まっている（D16。クロックセルの RCK=10kΩ は別枠、本型番はCK11=1uF・RC11=10kΩも使用）

## D. 製造データ

- [x] Gerber + ドリルを出力した（`kicad-cli pcb export gerbers`/`export drill`。**`74x273_tile.kicad_pcb`（非パネル、50×50mm）から**。ファイル: `74x273_tile_gerbers.zip`）
- [x] BOM を出力した（`kicad-cli sch export bom`。`74x273_tile.kicad_sch` から。列名 `Comment` / `Designator` / `Footprint` / `LCSC Part #`。ファイル: `74x273_bom.csv`）
- [x] CPL を出力した（`kicad-cli pcb export pos`。`74x273_tile.kicad_pcb` から。列 `Designator` / `Mid X` / `Mid Y` / `Layer` / `Rotation`。ファイル: `74x273_cpl.csv`）
- [x] CPL に BOM へ無い designator が含まれていない（`U1`・`J1`・基準マーク・取付穴を除外。本セッションで自動突合し完全一致（41件）を確認済み。Gerber・BOM・CPLとも同一の`74x273_tile`から出力しているため一致は自明）
...
すべて `pcb-demo/_carrier/tiles/74x273/` にある（非パネル・D26のため、Gerber/BOM/CPL/KiCadソースが全て同じ場所・同じタイル基準）。

- Gerber + ドリル: `pcb-demo/_carrier/tiles/74x273/74x273_tile_gerbers.zip`
- BOM: `pcb-demo/_carrier/tiles/74x273/74x273_bom.csv`
- CPL: `pcb-demo/_carrier/tiles/74x273/74x273_cpl.csv`
- KiCad ソース: `pcb-demo/_carrier/tiles/74x273/74x273_tile.kicad_{sch,pcb,pro}`
