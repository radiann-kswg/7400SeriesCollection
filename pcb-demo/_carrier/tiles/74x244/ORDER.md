...
- [x] 対象型番について VCC / GND のピン位置を一次資料で確認した（`_carrier/pinspec/74x244.json`。D15以前に確認済み、本セッションでは未変更）
- [x] 対象型番についてピン数を確認した（`DIP-20`。`load_part()` が生成時に検証）
- [x] `_carrier/pinspec/74x244.json` があり、各ピンの方向を一次資料で確認した
...
- [x] `Rd` が 220Ω（D16。470Ω 以下への変更なし。`74x244_bom.csv` で確認）
...
- [x] ERC エラー 0（本セッション、74x244 タイル再生成後に再実行）
...
- [x] Gerber + ドリルを出力した（`kicad-cli pcb export gerbers`/`export drill`。**`74x244_tile.kicad_pcb`（非パネル、50×50mm）から**。ファイル: `74x244_tile_gerbers.zip`）
- [x] BOM を出力した（`kicad-cli sch export bom`。`74x244_tile.kicad_sch` から。列名 `Comment` / `Designator` / `Footprint` / `LCSC Part #`。ファイル: `74x244_bom.csv`）
- [x] CPL を出力した（`kicad-cli pcb export pos`。`74x244_tile.kicad_pcb` から。列 `Designator` / `Mid X` / `Mid Y` / `Layer` / `Rotation`。ファイル: `74x244_cpl.csv`）
- [x] CPL に BOM へ無い designator が含まれていない（`U1`・`J1`・基準マーク・取付穴を除外。本セッションで自動突合し完全一致（37件）を確認済み。Gerber・BOM・CPLとも同一の`74x244_tile`から出力しているため一致は自明）
...
すべて `pcb-demo/_carrier/tiles/74x244/` にある（非パネル・D26のため、Gerber/BOM/CPL/KiCadソースが全て同じ場所・同じタイル基準）。

- Gerber + ドリル: `pcb-demo/_carrier/tiles/74x244/74x244_tile_gerbers.zip`
- BOM: `pcb-demo/_carrier/tiles/74x244/74x244_bom.csv`
- CPL: `pcb-demo/_carrier/tiles/74x244/74x244_cpl.csv`
- KiCad ソース: `pcb-demo/_carrier/tiles/74x244/74x244_tile.kicad_{sch,pcb,pro}`
