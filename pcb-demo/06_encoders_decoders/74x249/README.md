...
| -------------------- | ------------------ |
| **汎用型番**         | `74x249`  |
| **機能 (EN)**        | BCD to 7-seg decoder/driver (open-collector)    |
| **機能 (JP)**        | BCD-7セグデコーダ/ドライバ（オープンコレクタ） |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x249.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x249.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x249.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x249.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/06_encoders_decoders/74x249/`](../../../usage/06_encoders_decoders/74x249/)
...
- [ ] `usage/06_encoders_decoders/74x249/` のファクトチェックが完了している
