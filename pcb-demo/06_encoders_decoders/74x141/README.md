...
| -------------------- | ------------------ |
| **汎用型番**         | `74x141`  |
| **機能 (EN)**        | BCD to decimal decoder/driver for cold-cathode indicator or Nixie tube    |
| **機能 (JP)**        | BCD-10進デコーダ/ドライバー（冷陰極表示器/ニキシー管用） |
| **カテゴリ**         | `06_encoders_decoders`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | (なし)      |
| **入手済み (Other)** | K155ID1/К155ИД1    |

## KiCAD プロジェクト構成

| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x141.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x141.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x141.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x141.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/06_encoders_decoders/74x141/`](../../../usage/06_encoders_decoders/74x141/)
...
- [ ] `usage/06_encoders_decoders/74x141/` のファクトチェックが完了している
