...
| -------------------- | ------------------ |
| **汎用型番**         | `74x156`  |
| **機能 (EN)**        | BCD to decimal decoder with open-collector outputs    |
| **機能 (JP)**        | BCD-10進デコーダ（オープンコレクタ出力） |
| **カテゴリ**         | `06_encoders_decoders`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74156N, SN74LS156N, SN74ALS156N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x156.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x156.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x156.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x156.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/06_encoders_decoders/74x156/`](../../../usage/06_encoders_decoders/74x156/)
...
- [ ] `usage/06_encoders_decoders/74x156/` のファクトチェックが完了している
