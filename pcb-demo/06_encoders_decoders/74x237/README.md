...
| -------------------- | ------------------ |
| **汎用型番**         | `74x237`  |
| **機能 (EN)**        | 3-to-8 line decoder/demultiplexer with address latches    |
| **機能 (JP)**        | 3入力8出力デコーダ/デマルチプレクサ（アドレスラッチ付き） |
| **カテゴリ**         | `06_encoders_decoders`     |
| **入手済み (CMOS)**  | CD74HC237E     |
| **入手済み (TTL)**   | (なし)      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x237.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x237.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x237.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x237.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/06_encoders_decoders/74x237/`](../../../usage/06_encoders_decoders/74x237/)
...
- [ ] `usage/06_encoders_decoders/74x237/` のファクトチェックが完了している
