...
| -------------------- | ------------------ |
| **汎用型番**         | `74x845`  |
| **機能 (EN)**        | 8-bit D flip-flops, clear and set inputs    |
| **機能 (JP)**        | 8ビットD型フリップフロップ（CLR/SET入力付） |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x845.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x845.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x845.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x845.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/05_flipflops_latches/74x845/`](../../../usage/05_flipflops_latches/74x845/)
...
- [ ] `usage/05_flipflops_latches/74x845/` のファクトチェックが完了している
