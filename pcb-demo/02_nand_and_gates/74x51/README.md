...
| -------------------- | ------------------ |
| **汎用型番**         | `74x51`  |
| **機能 (EN)**        | Dual AND-OR-Invert gate (configuration depends on logic family)    |
| **機能 (JP)**        | 2回路AND-OR-インバートゲート（構成はロジックファミリに依存） |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x51.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x51.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x51.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x51.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/02_nand_and_gates/74x51/`](../../../usage/02_nand_and_gates/74x51/)
...
- [ ] `usage/02_nand_and_gates/74x51/` のファクトチェックが完了している
