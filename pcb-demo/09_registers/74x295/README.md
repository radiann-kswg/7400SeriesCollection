...
| -------------------- | ------------------ |
| **汎用型番**         | `74x295`  |
| **機能 (EN)**        | Quad 2-bit bidirectional universal shift register    |
| **機能 (JP)**        | 4回路2ビット双方向ユニバーサルシフトレジスタ |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74LS295BN      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x295.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x295.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x295.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x295.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x295/`](../../../usage/09_registers/74x295/)
...
- [ ] `usage/09_registers/74x295/` のファクトチェックが完了している
