...
| -------------------- | ------------------ |
| **汎用型番**         | `74x543`  |
| **機能 (EN)**        | Octal registered transceiver, non-inverting, three-state    |
| **機能 (JP)**        | 8ビットレジスタ付きトランシーバ（非反転、3状態） |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | 74F543N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x543.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x543.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x543.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x543.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x543/`](../../../usage/09_registers/74x543/)
...
- [ ] `usage/09_registers/74x543/` のファクトチェックが完了している
