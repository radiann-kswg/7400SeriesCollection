...
| -------------------- | ------------------ |
| **汎用型番**         | `74x173`  |
| **機能 (EN)**        | 4-bit D-type register with 3-state outputs    |
| **機能 (JP)**        | 4ビットD型レジスタ（3状態出力） |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | TC74HC173AF     |
| **入手済み (TTL)**   | SN74LS173AN, DM74173N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x173.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x173.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x173.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x173.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x173/`](../../../usage/09_registers/74x173/)
...
- [ ] `usage/09_registers/74x173/` のファクトチェックが完了している
