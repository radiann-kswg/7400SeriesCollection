...
| -------------------- | ------------------ |
| **汎用型番**         | `74x245`  |
| **機能 (EN)**        | Octal bus transceiver, 3-state outputs    |
| **機能 (JP)**        | 8ビットバストランシーバ（3状態出力） |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | SN74HC245N     |
| **入手済み (TTL)**   | 74FR245PC      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x245.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x245.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x245.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x245.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x245/`](../../../usage/09_registers/74x245/)
...
- [ ] `usage/09_registers/74x245/` のファクトチェックが完了している
