...
| -------------------- | ------------------ |
| **汎用型番**         | `74x195`  |
| **機能 (EN)**        | 4-bit parallel-access shift register    |
| **機能 (JP)**        | 4ビット並列アクセスシフトレジスタ |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | 74S195N, SN74195N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x195.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x195.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x195.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x195.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x195/`](../../../usage/09_registers/74x195/)
...
- [ ] `usage/09_registers/74x195/` のファクトチェックが完了している
