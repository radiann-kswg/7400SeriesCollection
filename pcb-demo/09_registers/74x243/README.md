...
| -------------------- | ------------------ |
| **汎用型番**         | `74x243`  |
| **機能 (EN)**        | quad bus transceiver, non-inverting outputs    |
| **機能 (JP)**        | 4回路バストランシーバ（非反転出力） |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | 74HC243AP     |
| **入手済み (TTL)**   | DM74S273N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x243.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x243.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x243.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x243.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x243/`](../../../usage/09_registers/74x243/)
...
- [ ] `usage/09_registers/74x243/` のファクトチェックが完了している
