...
| -------------------- | ------------------ |
| **汎用型番**         | `74x620`  |
| **機能 (EN)**        | Octal bus transceiver, inverting, 3-state outputs    |
| **機能 (JP)**        | 8ビットバストランシーバ（反転、3状態出力） |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | HD74HC620P     |
| **入手済み (TTL)**   | (なし)      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x620.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x620.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x620.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x620.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x620/`](../../../usage/09_registers/74x620/)
...
- [ ] `usage/09_registers/74x620/` のファクトチェックが完了している
