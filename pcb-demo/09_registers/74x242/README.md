...
| -------------------- | ------------------ |
| **汎用型番**         | `74x242`  |
| **機能 (EN)**        | Quad bus transceiver, inverting outputs    |
| **機能 (JP)**        | 4回路バストランシーバ（反転出力） |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | TC74HC242P     |
| **入手済み (TTL)**   | (なし)      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x242.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x242.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x242.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x242.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x242/`](../../../usage/09_registers/74x242/)
...
- [ ] `usage/09_registers/74x242/` のファクトチェックが完了している
