...
| -------------------- | ------------------ |
| **汎用型番**         | `74x379`  |
| **機能 (EN)**        | 4-bit register, clock enable and complementary outputs    |
| **機能 (JP)**        | 4ビットレジスタ（クロックイネーブル、補完出力） |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | MC74F379N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x379.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x379.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x379.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x379.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x379/`](../../../usage/09_registers/74x379/)
...
- [ ] `usage/09_registers/74x379/` のファクトチェックが完了している
