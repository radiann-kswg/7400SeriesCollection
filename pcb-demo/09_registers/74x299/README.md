...
| -------------------- | ------------------ |
| **汎用型番**         | `74x299`  |
| **機能 (EN)**        | 8-bit universal shift/storage register    |
| **機能 (JP)**        | 8ビットユニバーサルシフト/ストレージレジスタ |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74LS299N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x299.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x299.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x299.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x299.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x299/`](../../../usage/09_registers/74x299/)
...
- [ ] `usage/09_registers/74x299/` のファクトチェックが完了している
