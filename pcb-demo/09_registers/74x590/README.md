...
| -------------------- | ------------------ |
| **汎用型番**         | `74x590`  |
| **機能 (EN)**        | 8-bit binary counter with output register    |
| **機能 (JP)**        | 8ビットバイナリカウンタ（出力レジスタ付き） |
| **カテゴリ**         | `09_registers`     |
| **入手済み (CMOS)**  | 74HC590AP     |
| **入手済み (TTL)**   | (なし)      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x590.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x590.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x590.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x590.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/09_registers/74x590/`](../../../usage/09_registers/74x590/)
...
- [ ] `usage/09_registers/74x590/` のファクトチェックが完了している
