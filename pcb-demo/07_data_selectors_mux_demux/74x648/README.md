...
| -------------------- | ------------------ |
| **汎用型番**         | `74x648`  |
| **機能 (EN)**        | Octal bus transceiver/latch/multiplexer, inverting, three-state    |
| **機能 (JP)**        | 8ビット・バス・トランシーバ/ラッチ/MUX（反転、3状態） |
| **カテゴリ**         | `07_data_selectors_mux_demux`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74ALS648NT      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x648.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x648.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x648.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x648.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/07_data_selectors_mux_demux/74x648/`](../../../usage/07_data_selectors_mux_demux/74x648/)
...
- [ ] `usage/07_data_selectors_mux_demux/74x648/` のファクトチェックが完了している
