...
| -------------------- | ------------------ |
| **汎用型番**         | `74x251`  |
| **機能 (EN)**        | 8-input multiplexer, 3-state outputs    |
| **機能 (JP)**        | 8入力マルチプレクサ（3状態出力） |
| **カテゴリ**         | `07_data_selectors_mux_demux`     |
| **入手済み (CMOS)**  | SN74HC251N     |
| **入手済み (TTL)**   | SN74LS251N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x251.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x251.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x251.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x251.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/07_data_selectors_mux_demux/74x251/`](../../../usage/07_data_selectors_mux_demux/74x251/)
...
- [ ] `usage/07_data_selectors_mux_demux/74x251/` のファクトチェックが完了している
