...
| -------------------- | ------------------ |
| **汎用型番**         | `74x298`  |
| **機能 (EN)**        | Quad 2-input multiplexer/quad 2-bit bidirectional universal shift register    |
| **機能 (JP)**        | 4回路2入力マルチプレクサ/4回路2ビット双方向ユニバーサルシフトレジスタ |
| **カテゴリ**         | `07_data_selectors_mux_demux`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74LS298N, SN74298N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x298.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x298.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x298.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x298.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/07_data_selectors_mux_demux/74x298/`](../../../usage/07_data_selectors_mux_demux/74x298/)
...
- [ ] `usage/07_data_selectors_mux_demux/74x298/` のファクトチェックが完了している
