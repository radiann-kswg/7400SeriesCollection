...
| -------------------- | ------------------ |
| **汎用型番**         | `74x258`  |
| **機能 (EN)**        | Quad 2-to-1 multiplexer, inverting, tri-state    |
| **機能 (JP)**        | 4回路2入力マルチプレクサ（反転、3状態） |
| **カテゴリ**         | `07_data_selectors_mux_demux`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | 74F258APC, DM74AS258N, DM74LS258BN      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x258.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x258.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x258.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x258.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/07_data_selectors_mux_demux/74x258/`](../../../usage/07_data_selectors_mux_demux/74x258/)
...
- [ ] `usage/07_data_selectors_mux_demux/74x258/` のファクトチェックが完了している
