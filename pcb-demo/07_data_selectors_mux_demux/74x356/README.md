...
| -------------------- | ------------------ |
| **汎用型番**         | `74x356`  |
| **機能 (EN)**        | 8-to-1 data selector/multiplexer, edge-triggered registers    |
| **機能 (JP)**        | 8入力1出力マルチプレクサ（エッジトリガ型レジスタ） |
| **カテゴリ**         | `07_data_selectors_mux_demux`     |
| **入手済み (CMOS)**  | TC74HC356P, CD74HCT356E     |
| **入手済み (TTL)**   | (なし)      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x356.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x356.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x356.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x356.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/07_data_selectors_mux_demux/74x356/`](../../../usage/07_data_selectors_mux_demux/74x356/)
...
- [ ] `usage/07_data_selectors_mux_demux/74x356/` のファクトチェックが完了している
