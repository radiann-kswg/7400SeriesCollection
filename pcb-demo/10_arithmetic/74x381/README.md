...
| -------------------- | ------------------ |
| **汎用型番**         | `74x381`  |
| **機能 (EN)**        | 4-bit arithmetic logic unit(ALU), generate and propagate outputs    |
| **機能 (JP)**        | 4ビット算術論理演算装置（ALU、生成・伝播出力付き） |
| **カテゴリ**         | `10_arithmetic`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74LS381AN      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x381.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x381.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x381.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x381.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/10_arithmetic/74x381/`](../../../usage/10_arithmetic/74x381/)
...
- [ ] `usage/10_arithmetic/74x381/` のファクトチェックが完了している
