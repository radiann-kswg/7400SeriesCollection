...
| -------------------- | ------------------ |
| **汎用型番**         | `74x136`  |
| **機能 (EN)**        | Quad 2-input XOR gate with open-collector outputs    |
| **機能 (JP)**        | 4回路2入力XORゲート（オープンコレクタ出力） |
| **カテゴリ**         | `04_xor_xnor_gates`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74LS136N, HD74LS136P      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x136.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x136.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x136.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x136.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/04_xor_xnor_gates/74x136/`](../../../usage/04_xor_xnor_gates/74x136/)
...
- [ ] `usage/04_xor_xnor_gates/74x136/` のファクトチェックが完了している
