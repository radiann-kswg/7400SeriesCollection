...
| -------------------- | ------------------ |
| **汎用型番**         | `74x240`  |
| **機能 (EN)**        | Octal buffer/line driver, inverting, 3-state outputs    |
| **機能 (JP)**        | 8回路バッファ/ラインドライバ（反転、3状態出力） |
| **カテゴリ**         | `01_buffers_inverters`     |
| **入手済み (CMOS)**  | 74HC240AP     |
| **入手済み (TTL)**   | 74F240PC, SN74LS240N, SN74ALS240AN, MC74F240N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x240.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x240.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x240.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x240.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x240/`](../../../usage/01_buffers_inverters/74x240/)
...
- [ ] `usage/01_buffers_inverters/74x240/` のファクトチェックが完了している
