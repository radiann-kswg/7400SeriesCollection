...
| -------------------- | ------------------ |
| **汎用型番**         | `74x367`  |
| **機能 (EN)**        | Hex buffer/line driver, non-inverting, 3-state outputs    |
| **機能 (JP)**        | 6回路バッファ/ラインドライバ（非反転、3状態出力） |
| **カテゴリ**         | `01_buffers_inverters`     |
| **入手済み (CMOS)**  | SN74HC367N     |
| **入手済み (TTL)**   | 74367AN      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x367.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x367.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x367.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x367.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x367/`](../../../usage/01_buffers_inverters/74x367/)
...
- [ ] `usage/01_buffers_inverters/74x367/` のファクトチェックが完了している
