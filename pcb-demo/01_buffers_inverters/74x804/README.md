...
| -------------------- | ------------------ |
| **汎用型番**         | `74x804`  |
| **機能 (EN)**        | Octal buffer/line driver, 3-state outputs    |
| **機能 (JP)**        | 8ビットバッファ/ラインドライバ（3状態出力） |
| **カテゴリ**         | `01_buffers_inverters`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74ALS804BN      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x804.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x804.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x804.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x804.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x804/`](../../../usage/01_buffers_inverters/74x804/)
...
- [ ] `usage/01_buffers_inverters/74x804/` のファクトチェックが完了している
