...
| -------------------- | ------------------ |
| **汎用型番**         | `74x38`  |
| **機能 (EN)**        | Quad 2-input NAND buffer/driver with open-collector outputs    |
| **機能 (JP)**        | 4回路2入力NANDバッファ/ドライバ（オープンコレクタ出力） |
| **カテゴリ**         | `01_buffers_inverters`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74LS38N, MC74F38N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x38.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x38.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x38.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x38.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x38/`](../../../usage/01_buffers_inverters/74x38/)
...
- [ ] `usage/01_buffers_inverters/74x38/` のファクトチェックが完了している
