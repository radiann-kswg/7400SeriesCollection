...
| -------------------- | ------------------ |
| **汎用型番**         | `74x1244`  |
| **機能 (EN)**        | octal buffer / driver, non-inverting (lower-power version of 74x244)    |
| **機能 (JP)**        | 8回路バッファ/ドライバ（非反転、74x244の低消費電力版） |
| **カテゴリ**         | `01_buffers_inverters`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74ALS1244AN      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x1244.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x1244.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x1244.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x1244.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x1244/`](../../../usage/01_buffers_inverters/74x1244/)
...
- [ ] `usage/01_buffers_inverters/74x1244/` のファクトチェックが完了している
