...
| -------------------- | ------------------ |
| **汎用型番**         | `74x760`  |
| **機能 (EN)**        | Octal buffer/line driver, non-inverting, open-collector    |
| **機能 (JP)**        | 8ビット バッファ/ラインドライバ（非反転、オープンコレクタ） |
| **カテゴリ**         | `01_buffers_inverters`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | SN74AS760N, SN74ALS760N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x760.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x760.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x760.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x760.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x760/`](../../../usage/01_buffers_inverters/74x760/)
...
- [ ] `usage/01_buffers_inverters/74x760/` のファクトチェックが完了している
