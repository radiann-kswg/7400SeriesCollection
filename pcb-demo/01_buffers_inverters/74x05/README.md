...
| -------------------- | ------------------ |
| **汎用型番**         | `74x05`  |
| **機能 (EN)**        | Hex inverter with open-collector outputs    |
| **機能 (JP)**        | 6回路NOTゲート（オープンコレクタ出力） / 6回路インバータ（オープンコレクタ出力） |
| **カテゴリ**         | `01_buffers_inverters`     |
| **入手済み (CMOS)**  | SN74HC05N     |
| **入手済み (TTL)**   | SN74LS05N, SN74ALS05N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x05.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x05.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x05.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x05.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x05/`](../../../usage/01_buffers_inverters/74x05/)
...
- [ ] `usage/01_buffers_inverters/74x05/` のファクトチェックが完了している
