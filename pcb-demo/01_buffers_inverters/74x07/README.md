...
| -------------------- | ------------------ |
| **汎用型番**         | `74x07`  |
| **機能 (EN)**        | Hex buffer/drivers with open-collector outputs    |
| **機能 (JP)**        | 6回路バッファ/ドライバ（オープンコレクタ出力） |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x07.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x07.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x07.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x07.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x07/`](../../../usage/01_buffers_inverters/74x07/)
...
- [ ] `usage/01_buffers_inverters/74x07/` のファクトチェックが完了している
