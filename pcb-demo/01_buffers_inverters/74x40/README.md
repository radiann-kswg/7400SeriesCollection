...
| -------------------- | ------------------ |
| **汎用型番**         | `74x40`  |
| **機能 (EN)**        | Hex inverter buffer/driver with open-collector outputs    |
| **機能 (JP)**        | 6回路インバータバッファ/ドライバ（オープンコレクタ出力） |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x40.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x40.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x40.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x40.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x40/`](../../../usage/01_buffers_inverters/74x40/)
...
- [ ] `usage/01_buffers_inverters/74x40/` のファクトチェックが完了している
