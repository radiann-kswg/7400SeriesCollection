...
| -------------------- | ------------------ |
| **汎用型番**         | `74x125`  |
| **機能 (EN)**        | Quad bus buffer, tri-state outputs    |
| **機能 (JP)**        | 4回路バスバッファ（3状態出力） |
| **カテゴリ**         | `01_buffers_inverters`     |
| **入手済み (CMOS)**  | 74HC125P, SN74HC125N     |
| **入手済み (TTL)**   | SN74F125N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x125.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x125.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x125.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x125.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/01_buffers_inverters/74x125/`](../../../usage/01_buffers_inverters/74x125/)
...
- [ ] `usage/01_buffers_inverters/74x125/` のファクトチェックが完了している
