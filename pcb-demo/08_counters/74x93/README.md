...
| -------------------- | ------------------ |
| **汎用型番**         | `74x93`  |
| **機能 (EN)**        | 4-bit binary counter (separate divide-by-2 and divide-by-8 sections); different pinout for 74L93    |
| **機能 (JP)**        | 4ビットバイナリカウンタ（2剰余と8剰余でセクションを区切る）；74L93はピン配置が異なる |
| **カテゴリ**         | `08_counters`     |
| **入手済み (CMOS)**  | (なし)     |
| **入手済み (TTL)**   | DM74LS93N      |
| **入手済み (Other)** | (なし)    |
...
| ファイル                    | 役割                                             |
| --------------------------- | ------------------------------------------------ |
| `74x93.kicad_pro` | KiCAD プロジェクトファイル                       |
| `74x93.kicad_sch` | 回路図（スタブ — 要作成）                        |
| `74x93.kicad_pcb` | PCB レイアウト（50×50mm 外形済み）               |
| `74x93.kicad_sym` | カスタムシンボルスタブ（KiCAD 標準 74xx を推奨） |
...
- 回路設計情報（usage）: [`usage/08_counters/74x93/`](../../../usage/08_counters/74x93/)
...
- [ ] `usage/08_counters/74x93/` のファクトチェックが完了している
