# 74x173 — 4-bit D-type register with 3-state outputs
...
- [x] **対象の実型番を決める**（入手履歴より自動確認 2026-05-30）
  - CMOS系: `TC74HC173AF`
  - TTL系: `SN74LS173AN`, `DM74173N`
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TOSHIBA/TC74HC173AF.PDF`
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74LS/SN74LS173.PDF`
  - URL (SN74LS173AN (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74ls173a.pdf
- [ ] **ピン配置**（DIP/SOIC/TSSOP など）
- [ ] **真理値表/機能表**（イネーブル極性含む）
- [x] **絶対最大定格/推奨動作条件**（Vcc 範囲、入力電圧、I/O 電流 等）（データシートより自動確認 2026-05-30, 読み取り信頼度 100%）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74LS/SN74LS173.PDF`（一次資料）
