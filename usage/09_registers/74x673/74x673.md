# 74x673 — 16-bit serial-in, serial/parallel-out shift register with output storage, three-state
...
- **汎用型番**: `74x673`
- **カテゴリ**: `09_registers`
- **機能（EN）**: 16-bit serial-in, serial/parallel-out shift register with output storage, three-state
- **機能（JP）**: 16ビットSISO/SIPOシフトレジスタ（出力ストレージ付、3状態）
- **レア度**: `Maniac(SR+)`
