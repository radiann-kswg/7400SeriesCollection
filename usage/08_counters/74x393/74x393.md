# 74x393 — Dual 4-bit binary counter
...
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC393.PDF`
  - URL (SN74HC393N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74hc393.pdf
- [x] **ピン配置**（DIP/SOIC/TSSOP など）（データシートより自動確認 2026-05-30, 読み取り信頼度 86%）
  - 参照: (PDF 参照) Pin Configuration（一次資料）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC393.PDF`（一次資料）
- [ ] **真理値表/機能表**（イネーブル極性含む）
