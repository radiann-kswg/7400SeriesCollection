...
- [x] **対象の実型番を決める**（入手履歴より自動確認 2026-05-30）
  - CMOS系: `74HC02N`, `SN74HC02N`, `HD74HC02F`, `TC74HC02AF`
  - TTL系: `74S02N`, `SN74F02N`
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/PHILIPS(NXPSemiconductors)/74HC02.PDF`
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC02.PDF`
  - URL (SN74F02N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74f02.pdf
  - URL (SN74HC02N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74hc02.pdf
- [x] **ピン配置**（DIP/SOIC/TSSOP など）（データシートより自動確認 2026-05-30, 読み取り信頼度 86%）
  - VCC: Pin 4
  - 参照: (PDF 参照) Pin Configuration（一次資料）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC02.PDF`（一次資料）
- [ ] **真理値表/機能表**（イネーブル極性含む）
...
- [x] **未使用入力/未使用出力の扱い**（データシート推奨）（データシートより自動確認 2026-05-30, 読み取り信頼度 100%）
  - Unused inputs must be terminated at
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC02.PDF`（一次資料）
