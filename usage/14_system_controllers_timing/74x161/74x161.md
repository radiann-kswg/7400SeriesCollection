# 74x161 — 4-bit binary counter, synchronous, with clear and load
...
- [x] **対象の実型番を決める**（入手履歴より自動確認 2026-05-30）
  - CMOS系: `SN74HC161N`
  - TTL系: `SN74F161AN`, `SK74LS161`
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC161.PDF`
  - URL (SN74F161AN (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74f161a.pdf
  - URL (SN74HC161N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74hc161.pdf
- [ ] **ピン配置**（DIP/SOIC/TSSOP など）
- [ ] **真理値表/機能表**（イネーブル極性含む）
- [x] **絶対最大定格/推奨動作条件**（Vcc 範囲、入力電圧、I/O 電流 等）（データシートより自動確認 2026-05-30, 読み取り信頼度 100%）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC161.PDF`（一次資料）
