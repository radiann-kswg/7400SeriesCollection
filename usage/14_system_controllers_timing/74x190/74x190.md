# 74x190 — Synchronous up/down decade counter
...
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/CD74HC/CD74HC190,CD74HC191.PDF`
  - URL (CD74HC190E (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/cd74hc190.pdf
- [ ] **ピン配置**（DIP/SOIC/TSSOP など）
- [ ] **真理値表/機能表**（イネーブル極性含む）
- [x] **絶対最大定格/推奨動作条件**（Vcc 範囲、入力電圧、I/O 電流 等）（データシートより自動確認 2026-05-30, 読み取り信頼度 100%）
  - 参照: `datasheets/_download/TI(TexasInstruments)/CD74HC/CD74HC190,CD74HC191.PDF`（一次資料）
