# 74x666 — 8-bit D-type transparent read-back latch, non-inverting, three-state
...
- **機能（EN）**: 8-bit D-type transparent read-back latch, non-inverting, three-state
- **機能（JP）**: 8ビットD型透過リードバック・ラッチ（非反転、3状態）
- **レア度**: `SuperRare`
