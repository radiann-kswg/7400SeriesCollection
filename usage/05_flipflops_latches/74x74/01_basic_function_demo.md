# 74x74 フリップフロップ/ラッチ 基本動作デモ

> この資料はサンプル回路の解説です。**電気特性・最大定格・入力閾値などは必ず一次資料（データシート）で確認**してください。

## 回路概要

所持している `74x74` 系ICのうち、`SN74HC74N`（DIP想定）を使って、最小構成で基本動作を確認する回路です。
...
対象の実型番 `SN74HC74N` のデータシートPDFから、ピン名/番号を機械抽出しました（抽出元: `.temp/datasheet_fetch/generated_samples/TI/SN74HC74N.pdf`）。
...
- **実型番**: `SN74HC74N`
- **データシートURL**: https://www.ti.com/lit/ds/symlink/sn74hc74.pdf
- **PDF sha256（ローカル）**: `80795224f5f2aa51d52b2aa06ace58bcd860d937d49d0744f58a569d29c7d07b`
