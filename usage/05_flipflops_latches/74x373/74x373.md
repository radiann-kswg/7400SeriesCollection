# 74x373 — Octal transparent latch, 3-state outputs
...
- [x] **対象の実型番を決める**（入手履歴より自動確認 2026-05-30）
  - CMOS系: `74HC373AP`, `SN74HC373N`
  - TTL系: `SN74S373N`, `MC74F373N`
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC373.PDF`
  - URL (SN74HC373N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74hc373.pdf
  - URL (SN74S373N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74s373.pdf
- [x] **ピン配置**（DIP/SOIC/TSSOP など）（データシートより自動確認 2026-05-30, 読み取り信頼度 86%）
  - 参照: (PDF 参照) Pin Configuration（一次資料）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC373.PDF`（一次資料）
- [ ] **真理値表/機能表**（イネーブル極性含む）
