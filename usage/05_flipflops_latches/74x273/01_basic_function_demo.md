# 74x273 フリップフロップ/ラッチ 基本動作デモ

> この資料はサンプル回路の解説です。**電気特性・最大定格・入力閾値などは必ず一次資料（データシート）で確認**してください。

## 回路概要

所持している `74x273` 系ICのうち、`SN74HC273N`（DIP想定）を使って、最小構成で基本動作を確認する回路です。
...
対象の実型番 `SN74HC273N` のデータシートPDFから、ピン名/番号を機械抽出しました（抽出元: `.temp/datasheet_fetch/generated_samples/TI/SN74HC273N.pdf`）。
...
- **実型番**: `SN74HC273N`
- **データシートURL**: https://www.ti.com/lit/ds/symlink/sn74hc273.pdf
- **PDF sha256（ローカル）**: `dd1760bf71a4a334deeb95f623dc0b97b4f7302a94c22a040d01a24434ffc099`
