# 74x73 — Dual JK flip-flop, negative-edge-triggered, clear
...
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/CD74HC/CD74HC73.PDF`
  - URL (CD74HC73E (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/cd74hc73.pdf
- [ ] **ピン配置**（DIP/SOIC/TSSOP など）
- [ ] **真理値表/機能表**（イネーブル極性含む）
- [ ] **絶対最大定格/推奨動作条件**（Vcc 範囲、入力電圧、I/O 電流 等）
- [x] **電気特性**（VIH/VIL, VOH/VOL, IOL/IOH, tpd など）（データシートより自動確認 2026-05-30, 読み取り信頼度 87%）
  - 参照: `datasheets/_download/TI(TexasInstruments)/CD74HC/CD74HC73.PDF`（一次資料）
