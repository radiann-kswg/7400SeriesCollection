# 74x594 — 8-bit shift registers, serial-in, parallel-out, output latches, buffered
...
- **機能（EN）**: 8-bit shift registers, serial-in, parallel-out, output latches, buffered
- **機能（JP）**: 8ビットシフトレジスタ（シリアル入力/パラレル出力、出力ラッチ、バッファ付き）
- **レア度**: `Normal+`
