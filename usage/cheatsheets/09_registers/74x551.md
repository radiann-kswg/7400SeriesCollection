# 74x551 — Octal registered transceiver with status flags, inverting, three-state
...
- **汎用型番**: `74x551`
- **カテゴリ**: `09_registers`
- **機能（EN）**: Octal registered transceiver with status flags, inverting, three-state
- **機能（JP）**: ステータスフラグ付き8ビットレジスタ・トランシーバ（反転、3状態）
- **レア度**: `Maniac(SR+)`
