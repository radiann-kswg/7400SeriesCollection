# 74x695 — 4-bit binary counter/latch/multiplexer, sync & async clears, three-state
...
- **機能（EN）**: 4-bit binary counter/latch/multiplexer, sync & async clears, three-state
- **機能（JP）**: 4ビットバイナリ・カウンタ/ラッチ/MUX（同期/非同期クリア、3状態）
- **レア度**: `Normal+`
