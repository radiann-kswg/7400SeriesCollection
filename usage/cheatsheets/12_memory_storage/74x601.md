# 74x601 — Dynamic memory refresh controller, transparent/burst modes, for 64K DRAM
...
- **機能（EN）**: Dynamic memory refresh controller, transparent/burst modes, for 64K DRAM
- **機能（JP）**: DRAMリフレッシュコントローラ（透過/バースト動作、64K向け）
- **レア度**: `Maniac(SR+)`
