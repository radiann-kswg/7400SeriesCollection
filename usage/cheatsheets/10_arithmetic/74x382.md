# 74x382 — 4-bit arithmetic logic unit(ALU), ripple carry and overflow outputs
...
- **汎用型番**: `74x382`
- **カテゴリ**: `10_arithmetic`
- **機能（EN）**: 4-bit arithmetic logic unit(ALU), ripple carry and overflow outputs
- **機能（JP）**: 4ビット算術論理演算装置（ALU、リップルキャリー・オーバーフロー出力付き）
- **レア度**: `ManiacRare(SSR)`
