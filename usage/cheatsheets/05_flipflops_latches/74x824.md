# 74x824 — 9-bit D-type flip-flops with clear and clock enable, inverting inputs
...
- **機能（EN）**: 9-bit D-type flip-flops with clear and clock enable, inverting inputs
- **機能（JP）**: 9ビット D型フリップフロップ（クリア・クロックイネーブル付、反転入力）
- **レア度**: `Rare`
