# 74x67 — AND-gated J-K master-slave flip-flop, asynchronous preset and clear
...
- **機能（EN）**: AND-gated J-K master-slave flip-flop, asynchronous preset and clear
- **機能（JP）**: ANDゲート入力付きJKマスタスレーブ・フリップフロップ（非同期プリセット・クリア）
- **レア度**: `Maniac(SR+)`
