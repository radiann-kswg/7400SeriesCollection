# 74x368 — Hex buffer/line driver, inverting, 3-state outputs
...
- [x] **対象の実型番を決める**（入手履歴より自動確認 2026-05-30）
  - CMOS系: `SN74HC368N`
  - TTL系: `SN74368AN`, `DM74LS368AN`
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC368.PDF`
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74365A,SN74368A.PDF`
  - URL (SN74HC368N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74hc368.pdf
- [ ] **ピン配置**（DIP/SOIC/TSSOP など）
- [ ] **真理値表/機能表**（イネーブル極性含む）
- [x] **絶対最大定格/推奨動作条件**（Vcc 範囲、入力電圧、I/O 電流 等）（データシートより自動確認 2026-05-30, 読み取り信頼度 100%）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC368.PDF`（一次資料）
