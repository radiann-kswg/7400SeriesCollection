# 74x125 — Quad bus buffer, tri-state outputs
...
- [x] **対象の実型番を決める**（入手履歴より自動確認 2026-05-30）
  - CMOS系: `74HC125P`, `SN74HC125N`
  - TTL系: `SN74F125N`
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/PHILIPS(NXPSemiconductors)/74HC125.PDF`
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC125.PDF`
  - URL (SN74F125N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74f125.pdf
  - URL (SN74HC125N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74hc125.pdf
- [x] **ピン配置**（DIP/SOIC/TSSOP など）（データシートより自動確認 2026-05-30, 読み取り信頼度 86%）
  - VCC: Pin 4
  - 参照: (PDF 参照) Pin Configuration（一次資料）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC125.PDF`（一次資料）
- [ ] **真理値表/機能表**（イネーブル極性含む）
