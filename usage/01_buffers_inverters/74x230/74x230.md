# 74x230 — Dual 4-bit buffer/driver (one inverted, one non-inverted), neg enable
...
- **機能（EN）**: Dual 4-bit buffer/driver (one inverted, one non-inverted), neg enable
- **機能（JP）**: 2回路4ビットバッファ/ドライバ（1つ反転・1つ非反転）、負論理イネーブル
- **レア度**: `Rare`
