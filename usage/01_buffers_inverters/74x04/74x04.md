...
- [x] **対象の実型番を決める**（入手履歴より自動確認 2026-05-30）
  - CMOS系: `74HC04AP`, `74HCU04AP`, `SN74HC04N`, `MM74HC04N/MC74HC04N`
  - TTL系: `SN74H04N`, `SN74F04N`, `SN74AS04N`, `SN74ALS04BN`
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC04.PDF`
  - URL (SN74ALS04BN (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74als04b.pdf
  - URL (SN74AS04N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74as04.pdf
  - URL (SN74F04N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74f04.pdf
- [x] **ピン配置**（DIP/SOIC/TSSOP など）（データシートより自動確認 2026-05-30, 読み取り信頼度 86%）
  - VCC: Pin 6
  - 参照: (PDF 参照) Pin Configuration（一次資料）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC04.PDF`（一次資料）
- [ ] **真理値表/機能表**（イネーブル極性含む）
...
- [x] **未使用入力/未使用出力の扱い**（データシート推奨）（データシートより自動確認 2026-05-30, 読み取り信頼度 100%）
  - Unused inputs must be terminated at VCC or
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC04.PDF`（一次資料）
