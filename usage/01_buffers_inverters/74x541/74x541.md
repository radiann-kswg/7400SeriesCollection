# 74x541 — Octal buffer/line driver, non-inverting, 3-state outputs
...
- [x] **対象の実型番を決める**（入手履歴より自動確認 2026-05-30）
  - CMOS系: `74HC541AP`, `SN74HC541N`
  - TTL系: （なし）
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC541.PDF`
  - URL (SN74HC541N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74hc541.pdf
- [x] **ピン配置**（DIP/SOIC/TSSOP など）（データシートより自動確認 2026-05-30, 読み取り信頼度 86%）
  - 参照: (PDF 参照) Pin Configuration（一次資料）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC541.PDF`（一次資料）
- [ ] **真理値表/機能表**（イネーブル極性含む）
