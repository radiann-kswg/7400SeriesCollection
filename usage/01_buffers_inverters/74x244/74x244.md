# 74x244 — Octal buffer/line driver, non-inverting, 3-state outputs
...
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC244.PDF`
  - URL (SN74AS244N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74as244a.pdf
  - URL (SN74HC244N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74hc244.pdf
- [x] **ピン配置**（DIP/SOIC/TSSOP など）（データシートより自動確認 2026-05-30, 読み取り信頼度 86%）
  - 参照: (PDF 参照) Pin Configuration（一次資料）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC244.PDF`（一次資料）
- [x] **真理値表/機能表**（イネーブル極性含む）（データシートより自動確認 2026-05-30, 読み取り信頼度 100%）
    A=H → Y=L / A=L → Y=H（インバータ）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC244.PDF`（一次資料）
