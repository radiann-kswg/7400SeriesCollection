# 74x306 — 8-bit LV-TTL to GTL+ bus transceiver, three-state and open-collector
...
- **機能（EN）**: 8-bit LV-TTL to GTL+ bus transceiver, three-state and open-collector
- **機能（JP）**: 8ビットLV-TTL⇔GTL+バストランシーバ、3状態/オープンコレクタ
- **レア度**: `VeryNerd(UR)`
