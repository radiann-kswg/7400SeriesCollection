# 74x324 — voltage-controlled oscillator (or crystal controlled), enable, complementary outputs (analog)
...
- **機能（EN）**: voltage-controlled oscillator (or crystal controlled), enable, complementary outputs (analog)
- **機能（JP）**: 電圧制御（または水晶制御）発振器、イネーブル付、補完出力（アナログ）
- **レア度**: `SuperRare`
