# 74x238 エンコーダ/デコーダ 基本動作デモ

> この資料はサンプル回路の解説です。**電気特性・最大定格・入力閾値などは必ず一次資料（データシート）で確認**してください。

## 回路概要

所持している `74x238` 系ICのうち、`CD74HC238E`（DIP想定）を使って、最小構成で基本動作を確認する回路です。
...
対象の実型番 `CD74HC238E` のデータシートPDFから、ピン名/番号を機械抽出しました（抽出元: `datasheets/_download/TI(TexasInstruments)/CD74HC/CD74HC138,CD74HC238.PDF`）。
...
- **実型番**: `CD74HC238E`
- **PDF sha256（ローカル）**: `10cda382bc08fe2bd4db176b9b805b785ca50283fb56d0f47abd86aa0d441554`
