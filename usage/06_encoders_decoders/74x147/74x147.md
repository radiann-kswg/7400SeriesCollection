# 74x147 — 10-to-4 line priority encoder
...
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/CD74HC/CD74HC147.PDF`
  - URL (CD74HC147E (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/cd74hc147.pdf
- [x] **ピン配置**（DIP/SOIC/TSSOP など）（データシートより自動確認 2026-05-30, 読み取り信頼度 86%）
  - 参照: (PDF 参照) Pin Configuration（一次資料）
  - 参照: `datasheets/_download/TI(TexasInstruments)/CD74HC/CD74HC147.PDF`（一次資料）
- [ ] **真理値表/機能表**（イネーブル極性含む）
