# 74x148 — 8-to-3 line priority encoder
...
- [x] **データシート参照先**（自動確認 2026-05-30）
  - PDF: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC148.PDF`
  - URL (SN74HC148N (TI(TexasInstruments))): https://www.ti.com/lit/ds/symlink/sn74hc148.pdf
- [ ] **ピン配置**（DIP/SOIC/TSSOP など）
- [ ] **真理値表/機能表**（イネーブル極性含む）
- [x] **絶対最大定格/推奨動作条件**（Vcc 範囲、入力電圧、I/O 電流 等）（データシートより自動確認 2026-05-30, 読み取り信頼度 100%）
  - 参照: `datasheets/_download/TI(TexasInstruments)/SN74HC/SN74HC148.PDF`（一次資料）
