# 74x600 — Dynamic memory refresh controller, transparent/burst modes, for 4K/16K DRAM
...
- **機能（EN）**: Dynamic memory refresh controller, transparent/burst modes, for 4K/16K DRAM
- **機能（JP）**: DRAMリフレッシュコントローラ（透過/バースト動作、4K/16K向け）
- **レア度**: `Maniac(SR+)`
