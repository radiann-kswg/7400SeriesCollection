# 74x227 — 64-bit FIFO memory (16x4), sync, I/O ready (open-collector)
...
- **機能（EN）**: 64-bit FIFO memory (16x4), sync, I/O ready (open-collector)
- **機能（JP）**: 64ビットFIFO（16×4、同期、I/Oレディ、オープンコレクタ）
- **レア度**: `VeryNerd(UR)`
